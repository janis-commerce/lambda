...
module.exports = class StepFunction {
	 ...
	 * @returns
	 */
	static getParams(arn, name, client, data) {
		...
		if((typeof client === 'string' && !client.trim().length) || (client && typeof client !== 'string'))
		...
		const inputData = { ...client && { session: { clientCode: client } }, ...data && { body: data } };
	 ...
	 * @returns
	 */
	static async startExecution(arn, name, client, data) {
		const params = this.getParams(arn, name, client, data);
	 ...
	 *
	 * @param {string} arn The state machine ARN
	 * @param {<type>} extraParams The extra parameters to filter
	 * @return {Promise<object>} The state machine execution
	 */

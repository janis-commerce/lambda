...
const isObjectNotEmpty = require('./helpers/is-object-not-empty');
...
module.exports = class Invoker {
	 ...
	 * @returns {Promise<Object<InvokeResponse>>} Invocations result
	 */
	 static async serviceCall(serviceCode, functionName, body) {
		...
		const invokeParams = await this.getInvokeServiceParams(serviceCode, functionName);
	 ...
	 */
	 static async serviceClientCall(serviceCode, functionName, session, body) {
		...
		const [session] = this.validateAndEnsureClientSessions(session);

		body = isObjectNotEmpty(body) ? body : null;

		const [payload] = this.generateSessionsAndPayloadsCombinations([session], [body]);

		const invokeParams = await this.getInvokeServiceParams(functionName);
	 ...
	 */
	static async getPartialARN(serviceCode, functionName) {
		...
		return `${serviceAccountId}:function:${functionName}`;
	}
	 ...
	 * @static
	 */
	static async getInvokeServiceParams(serviceCode, functionName) {

		return {
			FunctionName: await this.getPartialARN(serviceCode, functionName),

...
module.exports = class Invoker {
	 ...
	 * @throws {LambdaError} With code NO_SERVICE_ACCOUNT_ID when the service code has no Account ID mapped
	 */
	static async getServiceAccountId(serviceCode) {
		...
		try {
			return await AccountsIdsByService.getAccountId(serviceCode);
		} catch(err) {

			// Preserve lambda's own error contract (LambdaError.codes.NO_SERVICE_ACCOUNT_ID) for consumers already catching it
			if(err instanceof AccountsIdsByServiceError && err.code === AccountsIdsByServiceError.codes.NO_SERVICE_ACCOUNT_ID)
	...
	static validateAndEnsureClientSessions(session) {
		...
		return sessions.map(s => {
			...
			return s instanceof ApiSession ? s : new ApiSession({ clientCode: s, serviceName: process.env.JANIS_SERVICE_NAME });
		});
	}

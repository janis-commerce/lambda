...
module.exports = class Invoker {
	...
	static async serviceSafeCall(serviceCode, functionName, body) {
		...
		const invokeResponse = await this.lambda.invoke({
			...invokeParams,
			...(payload && { Payload: payload })
		});
	 ...
	 * @static
	 */
	static formatInvokeResponse(invokeResponse) {

		const { StatusCode, FunctionError, Payload } = invokeResponse;

		return {
			statusCode: StatusCode,
			...FunctionError && { functionError: FunctionError },
			body: Payload ?
				(isObject(Payload) && JSON.parse(Payload)) || Payload : {}
		};
	}
};

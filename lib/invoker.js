...
module.exports = class Invoker {
	 ...
	 * @static
	 */
	static get isLocalEnv() {

		if(!this._isLocalEnv)
			this._isLocalEnv = process.env.JANIS_ENV === 'local';
	 ...
	 */
	static get lambda() {
		return Lambda.getInstance({ useLocalEndpoint: this.isLocalEnv });
	}
	...
	static getServiceLambdaInstance(serviceAccountId, serviceCode) {
		...
		if(!this.isLocalEnv)
			return Lambda.getInstanceWithRole(serviceAccountId);
		...
		return Lambda.getInstanceForLocalService(servicePort);
	...
	static getInvokeServiceParams(serviceAccountId, functionName) {
		...
		return {
			FunctionName: this.isLocalEnv ? getLambdaFunctionName(functionName) : this.getPartialARN(serviceAccountId, functionName),

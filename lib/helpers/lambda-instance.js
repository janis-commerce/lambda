...
module.exports = class LambdaInstance {
	...
	static getInstance({ useLocalEndpoint } = {}) {
		...
		if(!this._basicInstance)
			this._basicInstance = new Lambda(useLocalEndpoint && { endpoint: this.buildLocalEndpoint(process.env.MS_PORT) });
	...
	*/
	...
	static getInstanceForLocalService(port) {
		return new Lambda({ endpoint: this.buildLocalEndpoint(port) });
	}
};
